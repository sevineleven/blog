...
근데 만들면서 제일 오래 붙잡은 건 기능이 아니었다.

**이 화면을 prod에 띄우면서, 누가 들어올 수 있게 할 것인가.**
...
> **접근 통제를 인프라(보안 그룹)에서 슬랙으로 옮겼다. 비밀번호 대신 "내 슬랙 신원 + 지금 이 기기의 IP"가 열쇠다.**

이 한 줄이 이 글의 절반이다. 나머지 절반은 FCM 페이로드 이야기인데, 그건 뒤에서.
...
여기에 비개발자용 안전장치를 몇 개 더 얹었다. 발송 전에 **"대상자 추출"**로 "지금 보내면 몇 명한테 가요"를 먼저 보여주고, 즉시 발송뿐 아니라 **예약 발송**(시각 비우면 즉시)도 된다. 수천 명한테 한 번에 쏘면 시간이 걸리니까, 발송은 백그라운드로 돌리고 결과 화면이 진행률을 폴링해 %로 보여준다.
...
만들고 나서 보니, 이번 작업의 줄기는 하나였다.

**원래 백엔드(나)나 인프라(AWS 콘솔)를 거쳐야 하던 일을, 슬랙·백오피스로 끌어내려 당사자가 직접 하게 만든 것.**

문구를 고치는 일은 디자이너에게, 공지를 보내는 일은 운영자에게, 그리고 admin에 들어가는 일조차 보안 그룹을 만질 줄 아는 누군가를 거치지 않고, 슬랙을 쓸 줄 아는 팀원이면 누구든 직접 할 수 있게 됐다. 비밀번호 없는 문은 보안을 포기한 게 아니라, 자물쇠를 "지금 그 사람이 앉은 자리"로 바꾼 거였다.

개발자는 사용자한테 편한 UX를 만드는 사람이라고만 생각했다. 근데 이번에 해보니, **팀에게도 그 편함을 만들어줄 줄 알아야** 하는 것 같다. 같이 일하는 사람의 일을 코드로 덜어주는 것. 그게 좀 멋있는 일인 것 같다.

아, 그리고 내 블로그는 "4분 안에 읽히는 글"을 나름의 원칙으로 잡고 있었는데, 이 글은 한참 넘겼다. 할 말이 많았던 걸로 해두자.
