...
auth 도메인에 이런 케이스가 있었다. 같은 소셜 계정으로 첫 로그인이 동시에 여러 번 들어오는 경우다. 각 요청은 "이 소셜에 연결된 user가 없네, 그럼 만들자"를 실행하는데, 이게 동시에 일어나면 user가 중복으로 생기거나 UNIQUE 제약에 부딪혀 500이 난다. 평소엔 멀쩡하다가 트래픽이 몰리는 순간에만 터지는 부류라, 한 번 짜 두지 않으면 운영에서야 발견한다.
